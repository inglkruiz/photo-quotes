...
function getPhotoQuote() {
  Promise.all([
    ...
    const image  = responses[0],
      text = responses[1],
      authorEl = document.getElementsByClassName('Quote-author')[0],
      photographerEl = document.getElementsByClassName('App-photographer')[0],
      { user: {name: photographerName, links: { html: photographerLink } } } = image,
      utmParams = 'utm_source=inglkruiz&utm_medium=referral&utm_campaign=api-credit';
